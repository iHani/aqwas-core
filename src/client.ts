...
export class AqwasClient {
  ...
  subscribe(key: string, cb: (value: unknown, oldValue: unknown) => void): () => void {
  ...
  on<E extends ClientEvent>(event: E, cb: EventCallback<E>): () => void {
    ...
    return () => this.listeners.get(event)?.delete(cb as EventCallback<ClientEvent>)
  }
  ...
  private emit<E extends ClientEvent>(event: E, ...args: Parameters<EventCallback<E>>): void {
